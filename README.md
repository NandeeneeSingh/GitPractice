...
3. In order to save your changes you must stage and commit your changes. Run the command: git add .
4. Then run: git commit -m [message], in that order
5. In order for your edit to be saved to the repo, you must push it. Run the command: git push -u origin [feature-yourname] --> the -u flag links your local branch to the remote, so later on you will only have to run git push. 
...
#### TO DO: ####
1. In order to update the main branch, you must switch into the main branch. Run the command: git checkout main
2. In order to get the latest changes, run the command: git pull origin main
...
7. Switch from your feature-branch to the main repo, then pull the latest changes from main.
8. One partner should run the command to merge into the main branch FIRST. They should not get any issues.
9. Then, the other partner should run the command to merge. This will cause a merge conflict.
...
4. Once the file looks correct and you have manually deleted the <<<, ===, and >>> markers, save the file and run the command: git add conflict.md
5. Then, stage and commit by running the command: git commit -m [message]
6. Since you are already in main, there is no need to change branches. Simply run the command: git push origin main
